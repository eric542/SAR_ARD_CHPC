...
if [ ! $OKscenes_cnt -eq 0 ]; then
	...
	echo "  'Real' times:" ${rtimes[@]%.*}s
	echo "  'User' times:" ${utimes[@]%.*}s
	echo "  'System' times:" ${stimes[@]%.*}s
fi
...
sbatch --dependency=afterany:$jobid --time=10 --mem=100M --ntasks-per-node=1 --job-name=CPUtimes $tmp_script
